...
class Inicio:
    def __init__(self, ancho, alto, imagen_ruta):
        ...
        if(self._textura.id == 0):
            rl.trace_log(rl.LOG_WARNING, f"failed to load texture from {imagen_ruta}")

        print("Imagen Ruta: " + imagen_ruta)

        self._fondo = rl.Rectangle(0,0,ancho,alto)

        self._crear_partida = rl.Rectangle(
            (ancho/4) -10,
            alto/3,
            ancho/4,
            alto/6
        ...
        self._unirse_partida = rl.Rectangle(
            (ancho/2) + 10,
            alto/3,
            ancho/4,
            alto/6
    ...
    def dibujar(self):
        ...
        rl.draw_texture_ex(self._textura,(self._fondo.x,self._fondo.y),0.0,escala_ancho,rl.WHITE)

        rl.draw_rectangle_rec(self._crear_partida,rl.RED)
        rl.draw_text("Crear Partida", int(self._crear_partida.x+10),int(self._crear_partida.y+10),20,rl.WHITE)

        rl.draw_rectangle_rec(self._unirse_partida,rl.BLUE)
        rl.draw_text("Unirse a Partida", int(self._unirse_partida.x+10),int(self._unirse_partida.y+10),20,rl.WHITE)
