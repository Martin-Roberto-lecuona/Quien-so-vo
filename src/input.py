...
class InputField:
    def __init__(self, x, y, ancho, alto, text="") -> None:
        self._campo = rl.Rectangle(
            x,
            y,
            ancho,
            alto
    ...
    def dibujar(self):
        # color_modificado = rl.BLACK
        # color_modificado[3] = 0.5*255
        color_modificado = rl.Color(0,0,0,255)
        color_modificado.a = int(0.5*255)
        rl.draw_rectangle_rec(self._campo, color_modificado)
        rl.draw_text(self._text, int(self._campo.x + PADDING_X), int(self._campo.y + PADDING_Y), FONT_SIZE, rl.WHITE)
    ...
    def send_text_over_socket(self):
        self._text = ""
